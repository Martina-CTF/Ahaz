...
def register_user_threaded(request_data: UserRequest):
    ...
    register_user_ovpn(teamname=request_data.team_id, username=request_data.user_id)
    logger.debug("About to obtain config")
    config = obtain_user_ovpn_config(teamname=request_data.team_id, username=request_data.user_id)
    logger.debug("About to insert config into db")
    insert_user_vpn_config(teamname=request_data.team_id, username=request_data.user_id, config=config)
...
async def adduser():
    ...
    userExists = get_user_vpn_config(teamname=request_data.team_id, username=request_data.user_id)
...
def gen_team_from_flask_for_subprocess(request_data: RegisterTeamRequest) -> str:
    try:
        ...
        logger.debug("=9")
        insert_team_into_db(request_data.team_id)
        insert_vpn_port_into_db(request_data.team_id, request_data.port)
...
async def autogenerate_subprocess(request_data: UserRequest, port=-1) -> str:  # noqa: C901
    ...
    async def set_registration_progress_threaded(team_id: str, user_id: str, progress: int) -> None:
        set_registration_progress_team(team_id, user_id, progress)
    ...
    try:
        if get_registration_progress_team(request_data.team_id) == 10:
            return "team is being reregistered"
        logger.debug(get_registration_progress_team(request_data.team_id))
        if (
            get_registration_progress_team(request_data.team_id) == -999
            ...
            insert_team_into_db(request_data.team_id)
            insert_vpn_port_into_db(request_data.team_id, port)
            ...
            get_registration_progress_team(request_data.team_id) < 6
            ...
            while get_registration_progress_team(request_data.team_id) < 6:
                logger.info(f"waiting for team {request_data.team_id} user {request_data.user_id}")
                sleep(5)

            await set_registration_progress_threaded(request_data.team_id, request_data.user_id, 6)
        elif get_registration_progress_team(request_data.team_id) >= 6:  # if team is already registered, then
            await set_registration_progress_threaded(request_data.team_id, request_data.user_id, 6)

        teststatus = get_registration_progress_user(request_data.team_id, request_data.user_id)
        ...
        if get_registration_progress_team(request_data.team_id) == 10:
            return "team is being reregistered"
        if (get_registration_progress_user(request_data.team_id, request_data.user_id) == "null") or (
            get_registration_progress_user(request_data.team_id, request_data.user_id) == 6
            ...
            register_user_ovpn(request_data.team_id, request_data.user_id)

            await set_registration_progress_threaded(request_data.team_id, request_data.user_id, 8)
            logger.debug("about to obtain config")
            config = obtain_user_ovpn_config(request_data.team_id, request_data.user_id)
            logger.debug("about to insert config into db")
            insert_user_vpn_config(request_data.team_id, request_data.user_id, config)
...
async def autogenerate():
    ...
    status_user = get_registration_progress_user(request_data.team_id, request_data.user_id)
    ...
    status_team = get_registration_progress_team(request_data.team_id)
...
def del_team_subprocess(request_data: UserRequest | TeamRequest, reregister=False) -> None:
    ...
    delete_team_and_vpn(request_data.team_id)
